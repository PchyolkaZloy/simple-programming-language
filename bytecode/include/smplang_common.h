...
namespace bytecode {
    ...
    struct TokenPosition {
        TokenPosition(const size_t& line, const size_t& symbol)
            : line(line)
            , symbol(symbol) {
        }
    ...
    class ConstantFoldingException: public std::logic_error {
    public:
        explicit ConstantFoldingException(const std::string& message)
            : std::logic_error(message.c_str()) {
        }
    };
} // namespace bytecode